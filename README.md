...
**Now:** lexer (DolDoc `$` skip / `$IB`), preprocessor (`#include` `#define` `#if`),
parser (functions, control flow, chained compares, TempleOS operator precedence),
Cranelift JIT `--run`, `Print`/`PutChars`.

**Next:** `class` layout + field access, `Fs`/`Gs` TLS, 16-color `CDC`/`Gr*` in
`tos-gr`, DolDoc sprite bins, then `TempleOS/Demo/Games/Talons.HC`.
